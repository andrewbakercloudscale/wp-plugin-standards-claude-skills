...
- **Runtime PHP code generation — prohibited** — plugins must not assemble PHP source code as strings at runtime and write them to disk via `file_put_contents()`, `fwrite()`, or any WP Filesystem write call. This includes: constructing `<?php ... ?>` function bodies programmatically, building plugin headers dynamically, or interpolating variables into PHP syntax before writing. WordPress.org flags this as an arbitrary code-execution risk and an obfuscation concern regardless of whether the destination path is inside or outside the plugin directory. The correct pattern for files that must be deployed at runtime (e.g. a crash-test plugin, a drop-in, a mu-plugin): bundle them as static PHP files in `assets/` at build time and deploy via `copy( PLUGIN_DIR . 'assets/file.php', $dest )`. For runtime parameterisation, write the value to `update_option()` before copying and read it with `get_option()` inside the static file — the file on disk is always identical to the bundled source; nothing is interpolated at write time. **Audit:** `grep -n "file_put_contents\|fwrite" includes/*.php` — inspect every hit and confirm the content argument does not contain `<?php` or assemble PHP tokens as a string. Any such pattern is a hard violation.
