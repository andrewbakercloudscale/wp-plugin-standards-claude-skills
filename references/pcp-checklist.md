...
- [ ] No `parse_url()` — use `wp_parse_url()`. PCP flags `WordPress.WP.AlternativeFunctions.parse_url_parse_url` as an error on every native call. Audit: `grep -rn "parse_url(" *.php includes/*.php`.
...
- [ ] **`$_SERVER` URL superglobals** — `$_SERVER['HTTP_HOST']`, `$_SERVER['REQUEST_URI']`, `$_SERVER['SCRIPT_NAME']` fire `InputNotValidated`, `MissingUnslash`, and `InputNotSanitized`. **Preferred fix:** replace with WordPress helpers — `home_url()`, `admin_url()`, `wp_parse_url( home_url(), PHP_URL_HOST )`, `add_query_arg()`. These are already sanitised and environment-safe. Example: `$_SERVER`-based redirect URI construction → `add_query_arg( $args, admin_url( 'tools.php' ) )`. Audit: `grep -n "_SERVER\[" *.php includes/*.php`.
...
- [ ] Direct DB queries (`$wpdb->query()`, `$wpdb->get_results()`, etc.) require **three** sniff codes in the `phpcs:ignore` comment: `// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- [reason]`. `PluginCheck.Security.DirectDB.UnescapedDBParameter` fires when Plugin Check traces a variable used in the query string back to its assignment and cannot verify it came from a safe source. Fix options: (a) wrap `$wpdb->prefix`-derived table names with `esc_sql()` at assignment (`$table = esc_sql( $wpdb->prefix . 'tablename' );`) to satisfy the sniff without a suppression comment; (b) suppress with the ignore comment for SQL expressions built from internal conditionals. For blocks of queries use a `phpcs:disable` / `phpcs:enable` block — inline `phpcs:ignore` is not reliably respected on `$wpdb->query()` calls inside loops.
- [ ] Interpolated table names / internal column names in `$wpdb->prepare()` — suppress `WordPress.DB.PreparedSQL.InterpolatedNotPrepared` only when the interpolated value is a `$wpdb->prefix`-derived table name or an internal allowlist-validated column/expression (never user input). Pattern: `// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $table is $wpdb->prefix value; $cnt is internal enum`. For multi-line queries use `phpcs:disable` / `phpcs:enable` blocks. Plugins with many custom tables (analytics, stats) expect 50–100 of these warnings — plan suppression comments up front.
- [ ] **`PreparedSQLPlaceholders.ReplacementsWrongNumber`** — the number of `%d`/`%s`/`%f` placeholders in the `$wpdb->prepare()` query string does not match the number of replacement values passed. This is a real bug — fix the placeholder count; do not suppress.
- [ ] **`ExceptionNotEscaped`** — when `throw new \Exception($var)` or similar uses a variable in the message, PCP flags `WordPress.Security.EscapeOutput.ExceptionNotEscaped`. Exception messages are not HTML output. Suppress with `// phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- exception message is not HTML output`. For vendor library files, use `.phpcs.xml.dist` exclusion instead.
- [ ] **`slow_db_query_meta_key`** — PCP warns `WordPress.DB.SlowDBQuery.slow_db_query_meta_key` when `meta_key` appears in a `WP_Query`/`get_posts()` call. Ensure the `wp_postmeta` column is indexed (WordPress indexes `meta_key` by default) and suppress with `// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key -- meta_key column is indexed`.
...
- [ ] No `fopen()` / `fread()` / `fclose()` / `fwrite()` — use WP Filesystem API. PCP flags all four as `WordPress.WP.AlternativeFunctions.file_system_operations_fopen/fread/fclose/fwrite`. `fopen()` on remote URLs should use `wp_remote_get()`; for local files use `$wp_filesystem->get_contents()` / `$wp_filesystem->put_contents()`. Exception: third-party vendor libraries excluded via `.phpcs.xml.dist` (see below).
...
- [ ] **WP-Cron callbacks wrapped in `Throwable` guard — `try` as first statement** — every cron callback registered via `add_action( 'hook', $callback )` must wrap its **entire** body in `try { ... } catch ( \Throwable $e ) { error_log(...) }`. The `try {` must be the **first executable statement** inside the callback closure — any variable declaration or function call placed before `try` is unprotected and can crash the PHP-FPM worker on PHP 8. An inner `try/catch` covering only part of the body is not sufficient; the outer wrapper must start at line 1 of the callback. **Audit:** `grep -n "add_action.*scheduled\|csbr_scheduled\|wp_schedule_event" *.php includes/*.php` — open each result and confirm the first non-blank, non-comment line of the callback is `try {`.
...
- [ ] **`.phpcs.xml.dist` present if the plugin bundles third-party libraries** — Plugin Check runs PHPCS against every `.php` file in the zip, including `lib/`, `vendor/`, and similar subdirectory libraries. A `.phpcs.xml.dist` in the plugin root with `<exclude-pattern>*/lib/*</exclude-pattern>` / `<exclude-pattern>*/vendor/*</exclude-pattern>` lines tells Plugin Check to skip those directories. Missing this file produces dozens of false-positive `ExceptionNotEscaped`, `file_system_operations_fread`, `InterpolatedNotPrepared`, `NonPrefixedFunctionFound`, and `PluginCheck.Security.DirectDB.UnescapedDBParameter` errors from library code you cannot modify. **Note:** PCP's `missing_direct_file_access_protection` check runs outside PHPCS and is NOT suppressed by `.phpcs.xml.dist`. If this still fires on a vendor file, add `if ( ! defined( 'ABSPATH' ) ) { exit; }` on the line after `<?php` in that file — it is safe since the file is always loaded from within WordPress.
...
- [ ] **No files written to the plugin directory** — never write to any path under `plugin_dir_path()` or `WP_PLUGIN_DIR`. Plugin directories are deleted on upgrade; files stored there are lost and publicly accessible. Use `wp_upload_dir()['basedir'] . '/plugin-slug/'` for persistent data storage. Applies to: `copy()`, `file_put_contents()`, `fwrite()`, and any WP Filesystem write whose destination resolves to the plugin folder. PCP error code: **`PluginCheck.CodeAnalysis.WriteFile.PluginDirectoryWrite`** — note this same sniff also fires on `WP_CONTENT_DIR` (see next item), not just literal plugin-folder paths.
- [ ] **No raw `WP_CONTENT_DIR` for writable storage (root included)** — using `WP_CONTENT_DIR . '/plugin-slug/'` as a storage root is rejected, and so is writing to the `/wp-content` **root** directly (e.g. a companion `WP_CONTENT_DIR . '/slug-config.json'`) — this covers plain data files (JSON config, sidecar files), not just code. PCP flags **`PluginCheck.CodeAnalysis.WriteFile.PluginDirectoryWrite`** on `WP_CONTENT_DIR` fed to any write function (`copy()`, `file_put_contents()`, `fwrite()`, `fopen()` write-mode, `move_uploaded_file()`): "Detected usage of constant `WP_CONTENT_DIR`. Use `wp_upload_dir()` to get the uploads directory path or save to the database instead." Reviewers recognise only narrow `/wp-content` exceptions: `cache/`, `backups/`, and core drop-ins at their canonical path. All other writable directories (staging dirs, temp/chunk dirs, config/companion files) must use `wp_upload_dir()['basedir'] . '/plugin-slug/'`. Call `wp_upload_dir()` at runtime inside a function — never at file-load time. **Audit gap:** grep for `WP_CONTENT_DIR` AND read every function that returns a file path — the violation is often inside a helper (`function backup_dir() { return WP_CONTENT_DIR . '/...'; }`) that a grep-for-constant misses. Also grep for the plugin's directory name as a string literal to catch hardcoded paths.
- [ ] **No writes to system paths** — plugin code (including `uninstall.php`) must never write, create, chmod, or delete files at OS-level paths: `/usr/local/bin`, `/etc/cron.d`, `/var/log`, or any directory outside the WordPress installation. All persistent storage uses `wp_upload_dir()['basedir'] . '/plugin-slug/'`; logs use a file within that directory. Scheduled tasks use WP-Cron, not system cron. Server scripts that must reside on the OS cannot be auto-installed — provide copy-paste instructions to a server administrator. **Audit:** `grep -rn "/usr/local\|/etc/cron\|/var/log\|chmod\|chown" *.php includes/*.php uninstall.php`.
- [ ] **No executable code deployed to disk at runtime — generation OR `copy()`** — never (a) assemble PHP/shell source strings at runtime and write them via `file_put_contents()`/`fwrite()`/WP Filesystem, **nor** (b) `copy()`/`rename()`/`move_uploaded_file()` a bundled static `.php`/`.sh` file into a runtime location. Pattern (b) is **not** a valid workaround for (a) and is the most common repeat-rejection here: it is rejected to *every* destination, **including `wp_upload_dir()`** — the uploads dir holds data/media, never executable code. There is no compliant runtime-install path; features that must deploy a plugin/drop-in/script provide copy-paste install text for an administrator. **Audit:** `grep -n "file_put_contents\|fwrite\|\bcopy(\|\brename(\|move_uploaded_file" *.php includes/*.php` — hard violation if the content contains `<?php`/code tokens, OR a `copy()`/`rename()` destination is a `.php`/`.sh` filename (even from a bundled static source into uploads).
...
PCP and PHPCS only cover the automated half of review. The items below are the policy
rules a **human** reviewer enforces; they cause most repeat rejections of code that
already passes Plugin Check. Full detail and audit greps in
`references/wordpress-org-guidelines.md`.

- [ ] **No trialware** (G5) — no feature disabled until payment, no trial expiry, no
      licence-key gating of the free plugin's advertised functionality. `grep -rin "trial\|licen[sc]e_key\|is_pro\|expire\|unlock" --include=*.php`
- [ ] **No tracking without explicit opt-in** (G7) — every outbound call carrying site/
      user data (analytics, telemetry, activation check-in, newsletter signup) is gated
      behind a default-OFF opt-in and documented in `== External services ==`.
- [ ] **No "Powered by" / credit links by default** (G10) — any front-end attribution or
      outbound link is opt-in, default-hidden, never required to function.
- [ ] **No admin-dashboard hijacking** (G11) — admin notices are dismissible and shown
      only on the plugin's own screens; no site-wide nags, no dashboard ads with referral
      tracking. `grep -rn "admin_notices" --include=*.php`
- [ ] **Use WordPress's bundled libraries** (G13) — no own copy of jQuery, jQuery UI,
      Backbone, Underscore, SimplePie, PHPMailer, Requests, Masonry, MediaElement. `find . \( -iname "jquery*.js" -o -iname "*phpmailer*" -o -iname "*simplepie*" -o -iname "underscore*.js" -o -iname "backbone*.js" \)`
- [ ] **No external code/asset loading** (G8) — no third-party CDN for non-font assets,
      no admin pages in an external `<iframe>`, no installing plugins/themes from outside
      WordPress.org, no remote PHP includes. `grep -rn "<iframe\|cdn\.\|jsdelivr\|unpkg\|googleapis" --include=*.php --include=*.html`
- [ ] **Human-readable code** (G4) — no obfuscation; minified JS/CSS ships only if the
      unminified source is included or linked. `grep -rn "eval(\|base64_decode\|gzinflate\|str_rot13" --include=*.php`
- [ ] **GPL-compatible assets** (G1) — every bundled image, font, and library is
      GPL-compatible; third-party works listed under `== Credits ==`.
- [ ] **No readme spam** (G12) — ≤ 5 relevant tags, no competitor/trademark tags, no
      affiliate links, no keyword stuffing.
- [ ] **External API ToS** (G2) — programmatic use of every third-party API the plugin
      calls is permitted by that service's terms.
