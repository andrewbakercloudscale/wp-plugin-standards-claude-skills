...
- [ ] **No runtime PHP code generation** — never assemble PHP source strings at runtime and write them to disk via `file_put_contents()`, `fwrite()`, or WP Filesystem write calls. Bundle static PHP files in `assets/` and deploy via `copy()`. Use `get_option()` inside the static file for runtime parameters — the file on disk is always identical to the bundled source. **Audit:** `grep -n "file_put_contents\|fwrite" includes/*.php` — any hit whose content argument contains `<?php` or assembles PHP syntax as a string is a hard violation.
