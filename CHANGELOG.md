...
### Added
- `SKILL.md` — **`NonPrefixedClassFound` rule entry**: documents the case-sensitivity trap — PHPCS's `PrefixAllGlobals` sniff does an exact case match, so `CloudScale_Licence` is flagged as non-prefixed when the registered phpcs.xml prefix is lowercase `cloudscale`. Two fixes documented: (1) register all case variants (`cloudscale`, `CloudScale`, `CLOUDSCALE`) in the `<rule ref="WordPress.NamingConventions.PrefixAllGlobals">` block in `phpcs.xml`; (2) rename the class to match the registered prefix exactly. Same trap applies to `define()` constants and global function names under the same sniff.

## [1.2.4] - 2026-06-30

### Added
- `SKILL.md` — **Mandatory mechanical grep audit (Step 0.5)**: new required workflow step that runs ten deterministic bash greps immediately after the user confirms the review scope. Catches: `NonSingularStringLiteralDomain`, cURL, shell execution, `_e()`/`_ex()`, short tags, `application_detected` tooling files (initial six); plus `OutputNotEscaped` (echo/print with unescaped `$variables`), `parse_url` → `wp_parse_url`, `InputNotSanitized` (raw superglobal access), and `NonPrefixedClassFound` (class declarations for prefix verification). Root cause: these greps were documented as advisory text in rule descriptions but were not an enforced workflow step — the skill missed 34 `NonSingularStringLiteralDomain` violations, 25 `OutputNotEscaped` violations, 1 `parse_url_parse_url`, 2 `InputNotSanitized`, and 2 `NonPrefixedClassFound` warnings across two shared-class files.
